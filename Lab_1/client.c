...
int  main(int argc, char* argv[])
{
    ...
    /* make a socket */
    for (int count = 1; count <= times_to_download; count++) {
        ...
        //put this in a while loop, look at cs360 utils class.
        if (!debug && !countFlag) {
            std::string concat = "";
            int length = atoi(contentLength);
            while (nReadAmount < length) {
                nReadAmount+=read(hSocket,pBuffer,BUFFER_SIZE);
                concat.append(pBuffer);
            }
            printf("Response: \n\n%s\n", concat.c_str());
        }
        free(message);
    ...
    if (countFlag) {
        printf("\nDownloaded %d times successfully\n", times_to_download);
    }
